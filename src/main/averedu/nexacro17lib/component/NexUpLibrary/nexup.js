...
nfp._nexupCallback = function (o, nErrorcode, strErrorMsg)
 {
	...
	//CallBack 사용자 함수정의가 되어 있으면 호출
	var objApp = nexup.getApp();
	if(nexup.IsNull(objApp.gvNexUpCommCallBackFuncNm) == false){
		if(this[objApp.gvNexUpCommCallBackFuncNm] != undefined ){
			this[objApp.gvNexUpCommCallBackFuncNm].call(this, strSvcId, nErrorcode, strErrorMsg, fncCallback);
			return;
		}
	}		
...
/**
 * 중복된 Input Dataset을 보내야 하는경우 새로운 데이터셋으로 재정의 하여 보내기 위해 데이터셋을 화면에 추가
 * @return objEnv
 */
nexup.makeParamDataset = function(pThis, objOriginDs, sNewDatasetName)
{
	var dsName = sNewDatasetName;
	if(sNewDatasetName.indexOf(":") != -1){
		var arr = sNewDatasetName.split(":");
		dsName = arr[0];
	}
	
	var objDs = pThis.objects[dsName];
	if(nexup.IsNull(objDs)==true){
		objDs = new Dataset;
		objDs.set_name(dsName);
		pThis.addChild(objDs.name, objDs);
	}
	var nRowCnt = objDs.assign(objOriginDs);
	return sNewDatasetName;
}