package com.averedu.common.vo;

import java.sql.Date;
import java.util.List;

public class Csys310VO extends SearchVO{
	private int chk;
	private String bfDeptCd;
	private String deptAttrCd;
	private String deptAttrVal;
	private String remk;
	private String sortSeq;
	private String mngDept;
	private String cmmnCdYn;
	private String deptConnAttrCd;
	private String frstInputId;
	private Date frstInputDt;
	private String frstInputIp;
	private String lastModfId;
	private Date lastModfDt;
	private String lastModfMenuId;
	private String lastModfIp;
    private String deptConnAttrNm;
	public int getChk() {
		return chk;
	}
	public void setChk(int chk) {
		this.chk = chk;
	}
	public String getBfDeptCd() {
		return bfDeptCd;
	}
	public void setBfDeptCd(String bfDeptCd) {
		this.bfDeptCd = bfDeptCd;
	}
	public String getDeptAttrCd() {
		return deptAttrCd;
	}
	public void setDeptAttrCd(String deptAttrCd) {
		this.deptAttrCd = deptAttrCd;
	}
	public String getDeptAttrVal() {
		return deptAttrVal;
	}
	public void setDeptAttrVal(String deptAttrVal) {
		this.deptAttrVal = deptAttrVal;
	}
	public String getRemk() {
		return remk;
	}
	public void setRemk(String remk) {
		this.remk = remk;
	}
	public String getSortSeq() {
		return sortSeq;
	}
	public void setSortSeq(String sortSeq) {
		this.sortSeq = sortSeq;
	}
	public String getMngDept() {
		return mngDept;
	}
	public void setMngDept(String mngDept) {
		this.mngDept = mngDept;
	}
	public String getCmmnCdYn() {
		return cmmnCdYn;
	}
	public void setCmmnCdYn(String cmmnCdYn) {
		this.cmmnCdYn = cmmnCdYn;
	}
	public String getDeptConnAttrCd() {
		return deptConnAttrCd;
	}
	public void setDeptConnAttrCd(String deptConnAttrCd) {
		this.deptConnAttrCd = deptConnAttrCd;
	}
	public String getFrstInputId() {
		return frstInputId;
	}
	public void setFrstInputId(String frstInputId) {
		this.frstInputId = frstInputId;
	}
	public Date getFrstInputDt() {
		return frstInputDt;
	}
	public void setFrstInputDt(Date frstInputDt) {
		this.frstInputDt = frstInputDt;
	}
	public String getFrstInputIp() {
		return frstInputIp;
	}
	public void setFrstInputIp(String frstInputIp) {
		this.frstInputIp = frstInputIp;
	}
	public String getLastModfId() {
		return lastModfId;
	}
	public void setLastModfId(String lastModfId) {
		this.lastModfId = lastModfId;
	}
	public Date getLastModfDt() {
		return lastModfDt;
	}
	public void setLastModfDt(Date lastModfDt) {
		this.lastModfDt = lastModfDt;
	}
	public String getLastModfMenuId() {
		return lastModfMenuId;
	}
	public void setLastModfMenuId(String lastModfMenuId) {
		this.lastModfMenuId = lastModfMenuId;
	}
	public String getLastModfIp() {
		return lastModfIp;
	}
	public void setLastModfIp(String lastModfIp) {
		this.lastModfIp = lastModfIp;
	}
	public String getDeptConnAttrNm() {
		return deptConnAttrNm;
	}
	public void setDeptConnAttrNm(String deptConnAttrNm) {
		this.deptConnAttrNm = deptConnAttrNm;
	}

    
	
	
	
}
