...
/*
 ...
 ● gfn_commonUtils_report  : 웹경로 및 확장자를 포함한 리포트 출력
 ● gfn_commonUtils_report_ex  : 웹경로 및 확장자를 포함한 리포트 출력
 ● gfn_commonUtils_report2  : 웹경로 및 확장자를 포함한 리포트 출력
 ● gfn_getDayCount  		: 입력한 일수를 구분에 따라서 년, 월, 일 계산
*/
...
/**
 * 출력물 호출
 * @param {string} report - 웹경로 및 확장자를 포함한 리포트 파일 명
 * @param {Object} paramObj - 리포트에 전달 할 파라미터 오브젝트
 * 사용법 : app.gfn_commonUtils_report('SC/test.crf',  {param1:'A', param2:'B'})
 */
pForm.gfn_commonUtils_report = function (report, paramObj, isDirectPrint, multiYn) {
	const passphrase = 'MSTIS';
	var parameterQuery = '';
	
	var host = 'report.msu.ac.kr';
	/*if (location.hostname != 'mstis.msu.ac.kr') {
		host = 'reportdev.msu.ac.kr';
		host = 'localhost';
	} */
	
	var parameter = '';
	if (paramObj != undefined && paramObj != null && paramObj != '') {
		var keyArr = Object.keys(paramObj);
		if (typeof(paramObj[keyArr[0]]) === "object") {
			for (var idx = 0; idx < keyArr.length; idx++) {
				var keyArrSub = Object.keys(paramObj[keyArr[idx]]);
				parameter = '';
				for (var subIdx = 0; subIdx < keyArrSub.length; subIdx++) {
					parameter += ';' + keyArrSub[subIdx] + '=' + paramObj[keyArr[idx]][keyArrSub[subIdx]];
				}				
				parameter = parameter.substring(1);
				parameterQuery += '&' + keyArr[idx] + '=' + encodeURIComponent(CryptoJS.AES.encrypt(parameter, passphrase).toString());
			}
		} else {		
			for (var idx = 0; idx < keyArr.length; idx++) {
				parameter += ';' + keyArr[idx] + '=' + paramObj[keyArr[idx]];
			}
			parameter = parameter.substring(1);
			parameterQuery = '&' + report.substring(report.lastIndexOf('/') + 1, report.lastIndexOf('.')) + '=' + encodeURIComponent(CryptoJS.AES.encrypt(parameter, passphrase).toString());
		}
	}
	...
	param += 'en=1&';
	param += 'rpt=' + encodeURIComponent(CryptoJS.AES.encrypt(report, passphrase).toString());
	param += parameterQuery;
	
	window.open(location.protocol + "//" + host + "/view2.jsp?" + param, "", "width=1200, height=900, toolbar=no, location=no, status=no, menubar=no, scrollbars=yes, resizable=no");
}

pForm.gfn_commonUtils_report_ex = function (report, paramObj, isDirectPrint) {
	const passphrase = 'MSTIS';
...
*/	
	...
	param += 'en=1&';
	param += 'rpt=' + encodeURIComponent(CryptoJS.AES.encrypt(report, passphrase).toString()) + '&';
	param += 'prm=' + encodeURIComponent(CryptoJS.AES.encrypt(parameter, passphrase).toString());
	
	window.open(location.protocol + "//" + host + "/view1.jsp?" + param, "MSTIS Report", "width=900, height=900, toolbar=no, location=no, status=no, menubar=no, scrollbars=yes, resizable=no");
}

pForm.gfn_commonUtils_report2 = function (report, paramObj) {
	const passphrase = 'MSTIS';
	...
	*/
	...
	var url = location.protocol + "//" + host + "/view.jsp?" + param;
	
	return url;
}

/********************************************************************************
 * Function Name: gfn_getDayCount
 * Description  : 입력한 일수를 구분에 따라서 년, 월, 일 계산
 * Arguments	: day - 일수, gubn - 구분코드
 * Return 		: 계산된 날짜
 ********************************************************************************/
pForm.gfn_getDayCount = function(day,gubn) 
{
	var yy = 0;
	var mm = 0;
	var dd = 0;
	...
	if(day < 0) return -1;
	if (this.gfn_isNull(day)) return "";

	yyDay = day - (day % 360);
	yy = yyDay / 360;
	mmDay = (day - yyDay) - ((day - yyDay) % 30);
	mm = mmDay / 30;
	dd = day - yyDay - mmDay;
	...
	if(this.gfn_trim(sYy)=="" && this.gfn_trim(sMm)==""  && this.gfn_trim(sDd)=="" ){
		gubn = "99";
	}
